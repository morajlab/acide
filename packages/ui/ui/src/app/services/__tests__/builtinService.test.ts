...
describe('The Built-in Service', () => {
  ...
  test('Should support to get specific the built-in data', () => {
    const undefinedData = builtinService.getConstant('return-undefined' as any);
  ...
  test('[deprecated]Should support to inactive the specific constant', () => {
    ...
    expect(builtinService.getConstants()[inactiveConstant]).not.toBeUndefined();
    ...
    expect(builtinService.getConstant(inactiveConstant)?.active).toBe(false);
  });
