...
describe('Test Workbench Component', () => {
  ...
  test('Listen to The WorkbenchView onHorizontalPaneSizeChange event', async () => {
    const fn = jest.fn();
    render(
      <WorkbenchView {...workbenchModel()} onHorizontalPaneSizeChange={fn} />
    );
    const source = select<HTMLDivElement>('div[data-attribute="horizontal"]');
    if (source) {
  ...
  test('Hide the StatusBar', async () => {
    ...
    rerender(<WorkbenchView {...workbench} />);
    expect(select('.mo-statusBar')?.parentElement?.style.display).toBe('none');
  });
