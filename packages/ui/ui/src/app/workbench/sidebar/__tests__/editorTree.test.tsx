...
describe('The EditorTree Component', () => {
  ...
  test('Match Snapshot', () => {
    const component = renderer.create(<PaneEditorTree groups={mockGroups} />);
...
describe('The EditorTree Component With multiple groups', () => {
  ...
  test('Should focus the first child in group when click group title', () => {
    expectFnCalled((selectFn) => {
      ...
      const groups = container.querySelectorAll(`.${editorTreeGroupClassName}`);
      ...
      expect((group1.nextElementSibling! as HTMLDivElement).focus).toBeTruthy();
  ...
  test('Should support trigger contextMenu event in group title', () => {
    ...
    expectFnCalled((contextMenuFn) => {
      ...
      const group1 = container.querySelector(`.${editorTreeGroupClassName}`);
      ...
      expect(contextMenuFn.mock.calls[0][1]).toEqual(multipleGroups[0].id);
