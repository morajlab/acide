...
describe('Test MoleculeProvider', () => {
  ...
  test('MoleculeProvider should render built-in Workbench extensions', () => {
    ...
    expect(select('div[data-id="sidebar.explore.title"]')).toBeInTheDocument();
    expect(select('div[data-id="sidebar.search.title"]')).toBeInTheDocument();
    expect(select('.mo-welcome')).toBeInTheDocument();
    expect(select('div[id="statusbar.problems.title"]')).toBeInTheDocument();
  });
