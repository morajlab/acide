...
export function Collapse(props: ICollapseProps) {
  ...
  const filterData = visibleData.filter((panel) => panel.id) as ICollapseItem[];
  ...
  useLayoutEffect(() => {
    ...
    filterData.forEach((panel) => {
      ...
      if (isActive) {
        const contentDom =
          ...
          select(`.${collapseContentClassName}[data-content='${panel.id}']`);
    ...
    filterData.forEach((panel) => {
      const [height, top] = calcPosition(activePanelKeys, panel, filterData);
   ...
   */
  const getZeroPanelsByKeys = (keys: React.Key[], panels: ICollapseItem[]) => {
  ...
  const getContentHeightsByKeys = (data: React.Key[]) => {
    return data.map((key) => {
      ...
      const childrenDom = contentDom?.querySelector(`[data-content='${key}']`);
      ...
      const height = parseInt(contentHeight.toFixed(0)) - 2 + HEADER_HEIGTH;
  ...
  const calcPosition = (
    ...
    if (!isActive || panel._isEmpty) {
      ...
      if (panel.config?.grow === 0) {
        ...
        if (contentDom) {
          const height =
            contentDom.getBoundingClientRect().height + 2 + HEADER_HEIGTH;
          res[0] = height > MAX_GROW_HEIGHT ? MAX_GROW_HEIGHT : height;
        }
      } else {
        ...
        const nonEmptyAndActivePanelKeys = keys.filter((key) => {
          ...
          if (target) {
            ...
            if (typeof target._isEmpty === 'boolean') {
              !target._isEmpty && nonEmptyAndActivePanels.push(target);
        ...
        res[0] =
          ((wrapperHeight - emptyAndActivePanelsHeights) * getGrow(panel)) /
          growSum;
      }
    }
                ...
                <div className={collapseExtraClassName}>
                  {isActive && (
                    <Toolbar
                      key={panel.id}
                      data={panel.toolbar || []}
                      onClick={(e, item) => handleToolbarClick(e, item, panel)}
                    />
                  )}
                </div>
              </div>
