...
describe('The SearchPanel Component', () => {
  ...
  test('Should support to render title in result tree', () => {
    ...
    const { container, rerender } = render(
      <SearchPanel value={value} result={mockResult} getSearchIndex={mockFn} />
    );
    ...
    target = mockResult[1];
    treeNode = container.querySelector(`div[data-id=mo_treeNode_${target.id}]`);
    expect(treeNode).toBeInTheDocument();
    expect(
      treeNode!.querySelector(`.${matchSearchValueClassName}`)?.innerHTML
    ).toContain(value);
    ...
    expect(
      treeNode!.querySelector(`.${replaceSearchValueClassName}`)?.innerHTML
    ).toBe(replaceValue);
  });
