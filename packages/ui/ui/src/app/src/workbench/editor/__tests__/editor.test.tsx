...
jest.mock('mo/components/tabs', () => {
  ...
  return {
    ...originalModule,
    Tabs: (props) => {
      const { children, onSelectTab, onMoveTab, onCloseTab, ...others } = props;
      return (
        <div
          data-testid={TEST_ID}
          onClick={() => {
...
describe('The Editor Component', () => {
  ...
  test('Match the welcome snapshot', () => {
    const component = renderer.create(<Editor onClickActions={jest.fn()} />);
  ...
  test('Match the esitor group snapshot', () => {
    ...
    const component = renderer.create(
      <Editor onClickActions={jest.fn()} current={current} groups={groups} />
    );
    expect(component.toJSON()).toMatchSnapshot();
  });

  test('Match the status snapshot', () => {
    const component = renderer.create(<EditorStatusBarView {...mockItems} />);
    expect(component.toJSON()).toMatchSnapshot();
  });
  ...
  test('Should support to set single editor', () => {
    ...
    expect(container.querySelector(`.${groupClassName}`)).toBeInTheDocument();
  });
